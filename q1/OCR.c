...
int main() {
    ...
    int hidden_layer_nodes[HIDDENLAYERS+1]                                =  {0, 61, 29};
...
// --
    for (i = HIDDENLAYERS - 1; i > -1; i--) {
        for (j = 0; j <= i; j++) {
            hidden_layer_nodes[i] += hidden_layer_nodes[j];
        }           
    }
    ...
    for (i = NUMIN; i < NUMIN + NUMOUT + NUMHIDDEN; i++) {
            for (j = 0; j < NUMIN + NUMOUT + NUMHIDDEN; j++) { // iterates through the connections.               
                if (j <= NUMIN && i < NUMIN + i && (i - NUMIN - 1) < hidden_layer_nodes[1]) { // connects the inputs to the first layer.
                   weight[i][j] = 1; // Connect input nodes to hidden.
                } else if (j > hidden_layer_nodes[HIDDENLAYERS-1] + NUMIN -1 && j <= NUMIN+NUMHIDDEN && i >=NUMIN+NUMHIDDEN) {
                   weight[i][j] = 1; // Connect hidden nodes to output.
                } else if (j < hidden_layer_nodes[k] + NUMIN -1 && j > hidden_layer_nodes[k-1] + NUMIN -1 && i > hidden_layer_nodes[k] + NUMIN - 1 && i < hidden_layer_nodes[k+1] + NUMIN -1) {
                    weight[i][j] = 1;
                    if (j == hidden_layer_nodes[k])k++;
                } else {
                    break;
                }
            }
